...
api_name:
- CoreWebView2Profile
- CoreWebView2Profile.AreWebViewScriptApisEnabledForServiceWorkers
- CoreWebView2Profile.CookieManager
- CoreWebView2Profile.DefaultDownloadFolderPath
- CoreWebView2Profile.EnhancedSecurityModeLevel
- CoreWebView2Profile.EnhancedSecurityModeState
...
- CoreWebView2Profile.ServiceWorkerManager
- CoreWebView2Profile.SharedWorkerManager
...
- CoreWebView2Profile.GetEffectiveFeaturesForOriginAsync
...
Members|Description
--|--
[AreWebViewScriptApisEnabledForServiceWorkers](#arewebviewscriptapisenabledforserviceworkers) | Enables or disables WebView2 specific Service Worker JS APIs in the WebView2s associated with this profile.
...
[EnhancedSecurityModeLevel](#enhancedsecuritymodelevel) | 
[EnhancedSecurityModeState](#enhancedsecuritymodestate) | The `EnhancedSecurityModeState` property allows you to control whether Enhanced Security Mode is enabled or disabled for WebView2 instances associated with a profile. This applies to all WebViews sharing the same profile.
...
>  bool AreWebViewScriptApisEnabledForServiceWorkers

Enables or disables WebView2 specific Service Worker JS APIs in the WebView2s associated with this profile.
When set to `true`, `chrome` and `webview` objects are available in Service Workers. `chrome.webview` exposes APIs to interact with the WebView from Service Workers. The default value is `false`.
This setting applies to all newly installed Service Workers within the profile and is not persisted across WebView2 sessions.
...
>  [CoreWebView2EnhancedSecurityModeLevel](corewebview2enhancedsecuritymodelevel.md) EnhancedSecurityModeLevel

### EnhancedSecurityModeState

>  [CoreWebView2EnhancedSecurityModeState](corewebview2enhancedsecuritymodestate.md) EnhancedSecurityModeState

The `EnhancedSecurityModeState` property allows you to control whether Enhanced Security Mode is enabled or disabled for WebView2 instances associated with a profile. This applies to all WebViews sharing the same profile.

The configuration is not persisted to the user data folder. The property is reset when the profile is recreated.

The default value is [CoreWebView2EnhancedSecurityModeState.Disabled](corewebview2enhancedsecuritymodestate.md#disabled).

When set to `Disabled`, Enhanced Security Mode is disabled. When set to `Enabled`, Enhanced Security Mode is enabled.

See [CoreWebView2EnhancedSecurityModeState](corewebview2enhancedsecuritymodestate.md) for descriptions of states.
Changes apply immediately to new navigation. Existing pages require a reload for the change to take effect.

The EnhancedSecurityModeState setting is not persisted in the user data folder. It resets to the default value of [CoreWebView2EnhancedSecurityModeState](corewebview2enhancedsecuritymodestate.md).Disabled when the profile is destroyed and recreated.
...
### GetEffectiveFeaturesForOriginAsync

> [`IAsyncOperation`](/uwp/api/Windows.Foundation.IAsyncOperation-1)&lt;[`IVectorView`](/uwp/api/Windows.Foundation.Collections.IVectorView-1)&lt;[CoreWebView2OriginFeatureSetting](corewebview2originfeaturesetting.md)&gt;&gt; GetEffectiveFeaturesForOriginAsync(string origin)
...
> void SetOriginFeatures([`IIterable`](/uwp/api/Windows.Foundation.Collections.IIterable-1)&lt;string&gt; origins, [`IIterable`](/uwp/api/Windows.Foundation.Collections.IIterable-1)&lt;[`IKeyValuePair`](/uwp/api/Windows.Foundation.Collections.IKeyValuePair-2)&lt;[CoreWebView2OriginFeature](corewebview2originfeature.md), [CoreWebView2OriginFeatureState](corewebview2originfeaturestate.md)&gt;&gt; features)
