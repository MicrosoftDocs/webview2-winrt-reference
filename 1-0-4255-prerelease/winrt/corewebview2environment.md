...
- CoreWebView2Environment.CreateDiagnosticMonitor
- CoreWebView2Environment.CreateFindOptions
...
- CoreWebView2Environment.GetAvailableBrowserVersionString
- CoreWebView2Environment.GetAvailableBrowserVersionString
- CoreWebView2Environment.GetClusterEnvironmentOptions
...
[CreateDiagnosticMonitor](#creatediagnosticmonitor) | Creates a new diagnostic monitor. The monitor receives diagnostic signals from all layers - WebView, Profile, and Environment - that match its filters.
[CreateFindOptions](#createfindoptions) | 
[CreateOrJoinClusterEnvironmentAsync](#createorjoinclusterenvironmentasync) | Establishes or attaches to the shared cluster environment named by `options.ClusterName`, and returns a [CoreWebView2ClusterEnvironmentCreateResult](corewebview2clusterenvironmentcreateresult.md) describing the outcome.
...
[GetClusterEnvironmentOptions](#getclusterenvironmentoptions) | Synchronously reads the options of the cluster currently running for `clusterName`, without spawning a browser. Returns null when no cluster is running for that name, whether it was never created or its browser has since exited, and also when this host cannot use cluster environments at all.
...
> [CoreWebView2DiagnosticMonitor](corewebview2diagnosticmonitor.md) CreateDiagnosticMonitor()

Creates a new diagnostic monitor. The monitor receives diagnostic signals from all layers - WebView, Profile, and Environment - that match its filters.
Multiple monitors can coexist, each with its own filters and event handlers. The monitor is active immediately, but no events fire until a filter is set via [CoreWebView2DiagnosticMonitor.SetDiagnosticFilter](corewebview2diagnosticmonitor.md#setdiagnosticfilter). Release the monitor to stop receiving events and free resources.
...
> [CoreWebView2FindOptions](corewebview2findoptions.md) CreateFindOptions()



### CreateOrJoinClusterEnvironmentAsync

> static [`IAsyncOperation`](/uwp/api/Windows.Foundation.IAsyncOperation-1)&lt;[CoreWebView2ClusterEnvironmentCreateResult](corewebview2clusterenvironmentcreateresult.md)&gt; CreateOrJoinClusterEnvironmentAsync([CoreWebView2ClusterEnvironmentOptions](corewebview2clusterenvironmentoptions.md) options)

Establishes or attaches to the shared cluster environment named by `options.ClusterName`, and returns a [CoreWebView2ClusterEnvironmentCreateResult](corewebview2clusterenvironmentcreateresult.md) describing the outcome.
A cluster exists only while its browser process is running. The first host to establish a cluster for a given ClusterName fixes its options and starts the shared browser; once that browser exits the next host may establish a different set. The result's Status is Succeeded when the environment is ready, OptionsMismatch when a cluster is already running with different options, or NotSupported when this host cannot use cluster environments at all, for example a sandboxed AppContainer process such as a UWP app. The last two both report a null Environment and are outcomes rather than errors. On OptionsMismatch, read the running options with [CoreWebView2Environment.GetClusterEnvironmentOptions](corewebview2environment.md#getclusterenvironmentoptions) and retry, or use a private environment.
Errors are raised rather than reported as a Status. `options` must not be null, and E_INVALIDARG is raised when its ClusterName is missing or is not a valid cluster name. So handle errors for those, and inspect Status for everything above.
...
### GetClusterEnvironmentOptions

> static [CoreWebView2ClusterEnvironmentOptions](corewebview2clusterenvironmentoptions.md) GetClusterEnvironmentOptions(string clusterName)

Synchronously reads the options of the cluster currently running for `clusterName`, without spawning a browser. Returns null when no cluster is running for that name, whether it was never created or its browser has since exited, and also when this host cannot use cluster environments at all.
The value returned is a hint: it can be stale the instant it is read if the cluster's browser exits or another host establishes a different set concurrently. [CoreWebView2Environment.CreateOrJoinClusterEnvironmentAsync](corewebview2environment.md#createorjoinclusterenvironmentasync) remains authoritative and validates against the live cluster.
Raises E_INVALIDARG when `clusterName` is not a valid cluster name, which is distinct from the null returned for a name that is valid but not running.
